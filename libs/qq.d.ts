...
declare namespace qq {
     ...
     * 获取全局唯一的文件管理器
     */
    export function getFileSystemManager(): _qqFileSystemManager;

    export function downloadFile(obj: _qqDownLoadFile): void;

}

interface _qqFileSystemManager {
    access: (obj: _qqFileSystemManagerAccess) => {};

    unzip: (obj: _qqFileSystemManagerUnZip) => {};

    mkdir: (obj: _qqFileSystemManagerMkdir) => {};
}

interface _qqFileSystemManagerAccess {
    path: string;
    success?: (res) => void;
    fail?: (res) => void;
    complete?: (res) => void;
...
interface _qqFileSystemManagerUnZip {
    /**
     * 源文件路径，只可以是 zip 压缩文件
     */
    zipFilePath: string;
...
interface _qqFileSystemManagerMkdir {
    /**
     * 创建的目录路径
     */
    dirPath: string;
     ...
     * 如 dirPath 为 a/b/c/d 且 recursive 为 true，将创建 a 目录，
     * 再在 a 目录下创建 b 目录，以此类推直至创建 a/b/c 目录下的 d 目录
     */
    recursive?: boolean;
    success?: () => void;
    fail?: (res) => void;
